...
class VentasRepuestosWidget(QWidget):
    ...
    def _build_form(self) -> QScrollArea:
        ...
        self.items_table.setHorizontalHeaderLabels(["#", "Descripción del Artículo", "Precio ($)", "Cant.", "Subtotal ($)"])
        ...
        lbl_t_text = QLabel("TOTAL A PAGAR:")
        ...
        # ── Observaciones
        grp_obs, lay_obs = self._create_card("Observaciones")
        self.txt_obs = QTextEdit()
        self.txt_obs.setPlaceholderText("Notas adicionales sobre la venta, garantía...")
        ...
        self.btn_save.clicked.connect(self._save_venta)
    ...
    def _build_history(self) -> QWidget:
        ...
        self.search_venta = QLineEdit()
        self.search_venta.setPlaceholderText("🔍 Buscar…")
        self.search_venta.setFixedWidth(200)
        self.search_venta.setFixedHeight(36)
        self.search_venta.textChanged.connect(self.search_timer.start)
        hdr.addWidget(self.search_venta)
        lay.addLayout(hdr)
    ...
    def _collect_data(self) -> dict:
        items = []
        for row in range(self.items_table.rowCount()):
            d = self.items_table.item(row, 1)
            p = self.items_table.item(row, 2)
            q = self.items_table.item(row, 3)
    ...
    def _preview_thermal(self):
        ...
        if not data['cliente']:
            ...
            return
        text_content = build_venta_text(data)
    ...
    def _print_thermal(self):
        ...
        try:
            print_raw_text(build_venta_text(data))
    ...
    def _send_whatsapp(self):
        ...
        msg = (
            ...
            f"Su pedido de repuestos (Factura *{data['numero']}*) ya está listo para ser retirado.\n"
