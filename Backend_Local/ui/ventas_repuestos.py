...
import database
from printing.printer import build_venta_text, print_raw_text
...
class VentasRepuestosWidget(QWidget):
    ...
    def _build_form(self) -> QScrollArea:
        ...
        btn_new.setToolTip("Limpiar y crear nueva venta")
        btn_new.clicked.connect(self._new_venta)
        ...
        btn_import.setToolTip("Cargar datos de una venta ya hecha")
        ...
        # ── Observaciones
        grp_obs, lay_obs = self._create_card("Observaciones de la Venta")
        self.txt_obs = QTextEdit()
        self.txt_obs.setPlaceholderText("Notas adicionales, tiempo de garantía...")
        ...
        self.btn_save.setCursor(Qt.CursorShape.PointingHandCursor); self.btn_save.clicked.connect(self._save_venta)
    ...
    def _build_history(self) -> QWidget:
        ...
        hdr = QHBoxLayout()
        lbl = QLabel("📋  Historial de Ventas"); lbl.setStyleSheet("color:#e2e8f0;font-size:17px;font-weight:bold;")
    ...
    def _new_venta(self):
        self._saved_id = None
        now = datetime.now()
        self.txt_numero.setText(database.get_next_venta_number())
    ...
    def _import_from_existing(self):
        dlg = SelectorDialog("Importar Venta Existente", database.get_all_ventas, self)
        if dlg.exec():
    ...
    def _load_data(self, data: dict):
        ...
        QMessageBox.information(self, "Datos Cargados", "Se han importado los datos técnicos.")
    ...
    def _preview_thermal(self):
        ...
        self._show_text_preview(build_venta_text(data), f"Vista Previa — {data['numero']}")
    ...
    def _export_pdf(self):
        ...
        path, _ = QFileDialog.getSaveFileName(self, "Guardar PDF", f"{data['numero']}_venta.pdf", "PDF (*.pdf)")
        if path:
            text = build_venta_text(data)
    ...
    def _print_thermal(self):
        data = self._collect_data()
        if not data['cliente']: return
        if not self._saved_id: self._save_venta()
        try: print_raw_text(build_venta_text(data)); QMessageBox.information(self, "Impresión", "Ticket enviado.")
        except Exception as e: QMessageBox.critical(self, "Error", str(e))

    def _refresh_history(self):
        query = self.search_venta.text() if hasattr(self, 'search_venta') else ''
        ventas = database.get_all_ventas(query)
        ...
        for row, nota in enumerate(ventas):
            ...
            btn_ver.clicked.connect(lambda _, n=nota: self._show_text_preview(build_venta_text(n), f"Venta {n['numero']}"))
    ...
    def _send_whatsapp(self):
        ...
        msg = (f"Hola *{data['cliente']}*, le saluda *Llanos Core*. 👋\n\n"
               f"Su pedido de repuestos (Venta *{data['numero']}*) ya está listo.\nTotal: *${data['total']:.2f}*.\n\n¡Gracias!")
        import urllib.parse
